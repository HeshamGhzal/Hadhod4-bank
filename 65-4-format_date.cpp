...
sdate string_to_date(string date);
string convert_date_to_string(sdate date, string sperator);

string format_date(sdate date, string format = " dd / mm / yyyy");
...
int main()
{
	...
	date1 = string_to_date(date);

	cout << "day   " << date1.day << "\n";
	cout << "month " << date1.month << "\n";
	cout << "year  " << date1.year << "\n";
	cout << endl;
	cout << convert_date_to_string(date1, "/");
	cout << endl;
	cout << "---------------------\n";
	cout << format_date(date1) << "\n";
	cout << format_date(date1, " day dd / month mm / year yyyy") << "\n";
	cout << format_date(date1, " month mm / day dd / year yyyy") << "\n";
	cout << format_date(date1, " dd-mm-yyyy") << "\n";
	cout << format_date(date1, " yyyy -  mm - dd ") << "\n";
	cout << format_date(date1, " day : dd   month : mm   year : yyyy");
...
{
	sdate date1;
	vector<string> vs = spelt_each_word_in_string(date, "/");
	date1.day = stoi(vs[0]);
	date1.month = stoi(vs[1]);
	date1.year = stoi(vs[2]);
	return date1;
}
string convert_date_to_string(sdate date, string separator)
{
	string s;
	s += to_string(date.day) + separator;
	s += to_string(date.month) + separator;
	s += to_string(date.year);

	return s;
}
string replace_word_in_string_usig_biltin_fun(string s, string target, string new_word)
{
	int pos = s.find(target);
	string s2;
	while (pos != string::npos)
	{
		s2 = s.replace(pos, target.length(), new_word);
		pos = s.find(target);
	}
	return s2;
}

string format_date(sdate date, string format)
{
	vector<string> vdate = spelt_each_word_in_string(format, " ");

	format = replace_word_in_string_usig_biltin_fun(format, "dd", to_string(date.day));
	format = replace_word_in_string_usig_biltin_fun(format, "mm", to_string(date.month));
	format = replace_word_in_string_usig_biltin_fun(format, "yyyy", to_string(date.year));
	return  format;
}